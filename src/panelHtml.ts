/**
 * Pure webview HTML builder — no vscode dependency, so it can be rendered in a
 * real browser (Puppeteer) for automated CSP/behaviour testing (test/webview.ts),
 * in addition to being used by FsmPanel at runtime.
 */
import { ParsedFsm } from './parser';
import { escapeHtml } from './panelHelpers';

export type ThemeHint = 'auto' | 'light' | 'dark';

export function buildPanelHtml(fsms: ParsedFsm[], title: string, themeHint: ThemeHint, nonce: string): string {
